...
**Next.js 15 (App Router) + TypeScript (strict) + Tailwind + Vitest, deployed on Vercel.** Scaffolded and live at `https://aloud-implementation.vercel.app` (redeployed 2026-09-25). Plan Tasks 1–13 and the 2026-09-25 kinetic-caption work are done. **G1–G4 were re-run against the 2026-09-25 deployment** with `scripts/gate-probe.mjs` and passed. **G5 was not re-run**: the probe makes no `/v1/sessions/` call at all — it deletes only the probe *agent* — so the last verification of the recording deletion is 2026-09-22, by an uncommitted one-off. All five were closed as of 2026-09-22; four of them have evidence from 2026-09-25. What is genuinely left is **not code**: the submission demo video, and a human re-test of the browser matrix (`docs/BROWSER-NOTES.md`, reset to PENDING on 2026-09-25 after the capture worklet, playback, caption timeline and live screen all changed).
