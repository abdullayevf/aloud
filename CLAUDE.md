...
**Next.js 15 (App Router) + TypeScript (strict) + Tailwind + Vitest, deployed on Vercel.** Scaffolded and live at `https://aloud-implementation.vercel.app` (redeployed 2026-09-26). Plan Tasks 1–13, the 2026-09-25 kinetic-caption work and the 2026-09-26 caption amendment (loudness strip deleted, ink now per word) are done. **G3 and G4 were re-run against the 2026-09-26 deployment** with `scripts/gate-probe.mjs` and passed (G4 byte-identical on all three turns, G3 mean 384 ms); G1 and G2 were last run on 2026-09-25 and nothing since has touched the pass-through. **G5 was not re-run**: the probe makes no `/v1/sessions/` call at all — it deletes only the probe *agent* — so the last verification of the recording deletion is 2026-09-22, by an uncommitted one-off. All five were closed as of 2026-09-22; four of them have evidence from 2026-09-25 or later. What is genuinely left is **not code**: the submission demo video, and a human re-test of the browser matrix (`docs/BROWSER-NOTES.md`, reset to PENDING on 2026-09-25 and still pending after the 2026-09-26 caption change — the capture worklet, playback, caption timeline and live screen have all changed since the last pass).
...
- **No level meter, waveform or volume bar on the call screen.** A voice trace drawn from the hearing party's measured loudness shipped 2026-09-25 and was **deleted 2026-09-26** — `app/components/VoiceTrace.tsx`, `lib/audio/level-trace.ts`, the RMS block in `public/pcm-processor.js` and `MicCapture.start`'s `onLevel` param are all gone. It was an honest measurement and it was unnecessary: the turn label already said whose turn it was, in words, and an always-moving strip beside it spent the screen's entire motion budget restating that. The one real cost is recorded rather than glossed — it was the only signal that beat the ~1 s caption lag. Design spec §0 item 5, kinetic-captions spec §0a.
- **The ink is three runs and a stroke, not two runs.** `lib/caption-timeline.ts`'s `inkWords` splits the pending row into *said* / *the one word being spoken now* / *not yet reached*. The current word carries an underline that grows across it in **that word's own measured duration** (`end_ms - start_ms`: 517 ms for "reschedule", 81 ms for "to"), driven by a CSS animation whose duration and negative delay arrive as inherited custom properties — `animation-*` set inline on the span would not reach the `::after`. The rAF loop only sets React state when the *word* changes (2–4×/s); the travel itself is the compositor's, never React's. The measured timings have gaps between words, so `currentIndex` deliberately holds to the **next** word's `start_ms` — retiring it at `end_ms` blinks the stroke out between every pair of words. `inkSplit` is gone.
